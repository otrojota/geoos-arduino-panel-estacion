...
void loop() {
  ...
  if (estado == ST_INICIANDO) {
    if (wifi_error()) {wifi_iniciaConfig(); estado = ST_CONFIG;}
      ...
      wifi_silentConfig();
      estado = ST_ESTACION;
    } else if (digitalRead(CONFIG_BUTTON) == LOW) {wifi_iniciaConfig(); estado = ST_CONFIG;}
  } else if (estado == ST_ESTACION) {
    loop_estacion();
    if (digitalRead(CONFIG_BUTTON) == LOW) {wifi_iniciaConfig(); estado = ST_CONFIG;}    
  }
